...
$(document).ready(function(){
    $('.follow-btn').on('click', function(e){
        ...
        var movieSeq = "<%= movieSeq %>"; // JSP 변수 movieSeq를 JavaScript 변수로 가져옴
        var userId = "<%= userId %>"; // JSP 변수 userId를 JavaScript 변수로 가져옴
        var likeButton = $(this);

        // Ajax 호출
        $.ajax({
            type: 'POST',
            url: 'like_handler.jsp', // 좋아요 처리를 담당하는 JSP 파일 URL
            data: {
                movieSeq: movieSeq,
                userId: userId,
